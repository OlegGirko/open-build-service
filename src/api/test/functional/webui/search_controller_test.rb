...
class Webui::SearchControllerTest < Webui::IntegrationTest
  ...
  def test_basic_search_functionality
    ...
    visit File.join(root_path, 'search?search_text=basedistro')
    page.must_have_text(/Base.* contains official released updates/)

    visit File.join(root_path, 'search?search_text=basedistro3')
    page.must_have_text(/Base.* distro without update project/)

    visit File.join(root_path, 'search?search_text=kdebase')
